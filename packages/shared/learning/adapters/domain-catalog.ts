...
//
// This module is the composition root that closes that gap. Adding a domain is now ONE entry
// here — both clients and the planner pick it up with no further edits.
//
// What a new domain still owes, deliberately:
//   - a `LearningDomainAdapter`: which activity types it can plan, and how it wants a day's
//     minutes split between them. Both are read on every plan build. Nothing else — see the
//     interface for the four members that were removed for having no reader.
...
import { languageDomainAdapter, generalDomainAdapter } from './domain-adapters.ts'
 ...
 * vocabulary. `general` is the honest home for everything else the marketplace carries.
 *
 * `labor-law` was removed rather than fixed. It was one country's professional exam with zero
 * goals ever created, and its only distinguishing behaviour — `requireSourceGrounding: true` —
 * made its own remediation impossible to satisfy: the prompt demanded a citation while the
 * allowed-source list is drawn from `content_sources`, a table with no writer and no rows.
 */
