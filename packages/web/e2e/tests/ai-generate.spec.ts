 ...
 * Generation runs server-side on our provider key (edge fn `ai-generate`),
 * so the client no longer supplies a provider/API key. Set E2E_AI_GENERATION=1
 * to run the live-generation specs (requires the AI_GENERATION_PROVIDER_KEY
 * edge secret + free quota/credits on the test account).
 *
...
test.describe('AI Generate — Full Generation Flow', () => {
  test('full flow: generate template + deck + cards on the server', async ({ page }) => {
    ...
    // 5. Set card count
    const cardCount = dialog.locator('input[type="number"]')
    await cardCount.fill('10')
    ...
    // 7. Wait for template generation (AI call — may take time)
    ...
    // 8. Template review step — verify fields exist
    ...
    // 16. Wait for save completion → Done step (use h3 specifically to avoid strict mode violation with DialogTitle)
    await expect(dialog.locator('h3').filter({ hasText: /Done!|완료!/i })).toBeVisible({ timeout: 30_000 })

    // 17. Verify done step shows summary
    await expect(dialog.locator('text=/cards? created|카드가 생성/i')).toBeVisible()

    // 18. Click "View Deck" to navigate
    ...
    // 19. Should navigate to deck detail page
    await page.waitForURL(/\/decks\/[a-z0-9-]+/, { timeout: 10_000 })

    // 20. Verify cards appear in the deck
    await page.waitForTimeout(2000)
