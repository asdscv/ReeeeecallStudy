...
const STATUS_COLORS: Record<string, string> = {
  ...
  reviewing: 'bg-brand/15 text-brand',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {pendingCount > 0 && (
              <span className="px-2 py-0.5 text-xs font-medium bg-destructive/15 text-destructive rounded-full">
        ...
        {reportsLoading ? (
          <div className="p-6 text-center text-content-tertiary text-sm">
            {t('loading', { defaultValue: 'Loading...' })}
          </div>
        ) : reportsError ? (
          <div className="p-6 text-center text-destructive text-sm">{reportsError}</div>
        ) : reports.length === 0 ? (
          <div className="p-6 text-center text-content-tertiary text-sm">
              ...
              <div key={report.id} className="px-4 py-3">
                <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {report.admin_note && (
                      <p className="text-xs text-brand mt-1 italic">Admin: {report.admin_note}</p>
                    )}
                    ...
                    {noteForReport === report.id && (
                      <input
                        ...
                        className="mt-2 w-full px-3 py-1.5 border border-border rounded-lg text-xs focus:border-brand outline-none"
                      />
                    )}
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {shareModePieData.length > 0 && (
          <div className="bg-card rounded-xl border border-border p-3 sm:p-5">
            <h3 className="text-sm font-medium text-foreground mb-3">{t('market.shareByMode')}</h3>
        ...
        {stats?.share_by_mode && stats.share_by_mode.length > 0 && (
          <div className="bg-card rounded-xl border border-border p-3 sm:p-5">
            <h3 className="text-sm font-medium text-foreground mb-3">{t('market.shareBreakdown')}</h3>
            <div className="space-y-3">
              {stats.share_by_mode.map((s) => {
                const total = stats.share_by_mode!.reduce((acc, m) => acc + m.count, 0)
                const pct = total > 0 ? Math.round((s.count / total) * 100) : 0
                return (
                  <div key={s.mode} className="p-3 bg-muted rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-foreground">{t(shareModeLabel(s.mode), s.mode)}</span>
                      <span className="text-sm font-semibold text-foreground">{s.count.toLocaleString()}</span>
                    </div>
                    ...
                    <p className="text-[11px] text-content-tertiary mt-1">{pct}%</p>
                  </div>
                )
              })}
            </div>
      ...
      {categoryBarData.length > 0 && (
        <div className="bg-card rounded-xl border border-border p-3 sm:p-5">
          <h3 className="text-sm font-medium text-foreground mb-3">{t('market.topCategories')}</h3>
