  ...
  return (
    <div className="bg-card rounded-xl border border-border p-3 sm:p-5">
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <h3 className="text-sm font-medium text-foreground">{t('charts.ratingDistribution')}</h3>
        <span className="text-xs text-content-tertiary">{t('charts.ratingTotal', { count: grandTotal })}</span>
      </div>
      {grandTotal === 0 ? (
        <p className="text-sm text-content-tertiary py-8 text-center">{t('charts.noRatingData')}</p>
      ) : (
    ...
    <div>
      {showLabel && (
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-xs font-semibold text-muted-foreground">{t(group.i18nKey)}</span>
          <span className="text-[10px] text-content-tertiary">{group.total}</span>
        </div>
      )}
        ...
        <div className="flex-1 space-y-1.5">
          {group.ratings.map((entry) => (
            <div key={entry.rating} className="flex items-center gap-2">
              ...
              <span className="text-xs text-content-tertiary w-10 text-right">{entry.percentage}%</span>
            </div>
          ))}
        </div>
      </div>
