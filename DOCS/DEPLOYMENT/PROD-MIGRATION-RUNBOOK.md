...
develop→main PR에는 **Migration promotion gate** 잡이 붙습니다. 이 승격이 main에 없는
마이그레이션을 싣는데 PR 설명에 적용 선언이 없으면 **머지를 막습니다.**

적용을 마친 뒤 PR 설명에 한 줄 넣으십시오:

```
prod-migrations-applied: 186
```

숫자는 이 승격이 싣는 **가장 높은 마이그레이션 번호**와 같아야 합니다. 낡은 번호를 적으면
(예: 183을 적었는데 186까지 싣는 경우) 남은 범위를 지목하며 다시 막습니다.

**이건 선언이지 측정이 아닙니다.** 프로덕션의 스키마 버전은 git에 없고, 진짜로 확인하려면
public 저장소 CI에 DB 자격증명을 넣어야 합니다 — 러너에서 임의 명령 실행 구멍이 나온 직후라
그 교환은 좋지 않습니다. 실제로 일어나는 실수(단계 자체를 잊거나, 이전 범위만 적용하고 더
높은 걸 승격하는 것)는 이걸로 잡힙니다.
