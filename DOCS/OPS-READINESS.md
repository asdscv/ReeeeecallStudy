...
|---|---|---|
| **Pack B admin UI** (quota/card/margin input fields) | Convenience — the RPCs are live, so any value is changeable now (ask Claude / SQL). | Add fields to Admin → System calling `admin_set_ai_free_quota` / `admin_set_card_limit` / `set_ai_pricing_settings`. ~1–2h. |
| **Web ConfigStep reads server free_limit** | Minor display drift only if you raise the quota above 10 (server already enforces the real value). | Replace `FREE_DAILY_CAP = 10` with `get_ai_generation_quota().free_limit`; same for mobile AIGenerateScreen. |
