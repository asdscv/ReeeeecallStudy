-- 213: two of the coach's six levers have never been able to fire.
--
-- `get_plan_digest` reports the goal's intake so the chooser can refuse a lever already at its
-- floor. It read `settings->>'new_cards_per_day'`. Nothing has ever written that key: both goal
-- forms save `newCardsPerDay`, and `parseNewCardsPerDay` reads `newCardsPerDay`. So the
-- projection returned NULL for every goal in existence, and `lower_intake` / `raise_intake` —
-- the two levers whose dial is `new_cards_per_day` — were unreachable in production.
--
-- Verified against production before writing this: three goals, and `count(settings->>'new
-- _cards_per_day')` is 0. So is the camel count, because the field only lands when a goal is
-- created or edited through the form since it shipped. Both spellings are accepted here so no
-- learner has to re-save a goal for the coach to start working.
--
-- The write side is fixed in the same change (`applyPlanCoach`), and it was the worse half:
-- accepting a lever posted `{ new_cards_per_day: value }` through `update_learning_goal`, whose
-- body is `settings = COALESCE(p_settings, settings)` — a whole-object REPLACE. Taking the
-- coach's advice therefore deleted `cadence`, silently resetting a 주7일 learner to the default
-- schedule, in exchange for writing an intake value nothing reads.
--
-- Body is 209's verbatim apart from the projection at the end.
BEGIN;
