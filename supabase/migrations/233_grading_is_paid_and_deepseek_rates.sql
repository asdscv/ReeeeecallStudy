-- 233: grading is never free, and DeepSeek's real models get rates.
--
-- ── Grading ────────────────────────────────────────────────────────────────
--
-- The run screen was offering "무료로 채점돼요 · 27번 더 무료". The free daily units and the
-- one-time trial exist to get a learner to their first quiz — to pay for the QUESTIONS, which
-- are the thing they cannot try without and which are bought once. Grading is a fresh model call
-- on every answer of every sitting, so a free allowance there is an open-ended subsidy on the one
-- action that repeats without limit, and the app was advertising it.
--
-- Free and trial units now cover `generate_*` only. `grade_*` is paid from the first one, in both
-- the quote and the reservation — they must agree, or the screen quotes a price the server then
-- refuses (P0008, "price changed since the quote").
--
-- ── DeepSeek rates ─────────────────────────────────────────────────────────
--
-- `GET /models` on the live key returns exactly `deepseek-v4-flash` and `deepseek-v4-pro`. The
-- `deepseek-chat` row here names a model that no longer exists.
--
-- These rates drive COST and margin reporting only — never what a learner is charged, which has
-- been a fixed list price since mig 216. They are provisional: DeepSeek publishes no rate card
-- through the API, so flash is entered at the old `deepseek-chat` rate, which for a cheaper model
-- is an UPPER bound. Margin is therefore understated rather than overstated, which is the safe
-- direction to be wrong in, and `note` says so.
BEGIN;
...
BEGIN
  ...
  v_free_left := GREATEST(0, s.free_quiz_units_per_day - COALESCE(v_free_used, 0));
    ...
    -- GRADING IS NEVER FREE.
  --
  -- The free daily units and the one-time trial exist to get a learner to their first quiz — to
  -- pay for the QUESTIONS, which are the thing they cannot try without. Grading is a fresh model
  -- call on every answer of every sitting, so a free allowance there is an open-ended subsidy on
  -- the one action that repeats without limit: "무료로 채점돼요 · 27번 더 무료" was the app
  -- advertising exactly that.
  --
  -- So free and trial units are offered to `generate_*` only. `grade_*` is paid from the first
  -- one, which is also the honest thing for the screen to say.
  IF p_action LIKE 'grade\_%' THEN
    -- NAMED columns. `SELECT 0, 0, v_total INTO v_alloc` gives the record unnamed fields, and
    -- every later `v_alloc.paid_units` then raises "record has no field" — which reached
    -- production for the minutes between applying this and reading a quote back.
    SELECT 0 AS trial_units, 0 AS free_units, v_total AS paid_units INTO v_alloc;
  ELSE
...
VALUES
  ('deepseek', 'deepseek-v4-flash', 270000, 1100000,
   'PROVISIONAL. Entered at the old deepseek-chat rate, an upper bound for a cheaper tier, so '
   'margin is understated rather than overstated. Replace when a published rate card exists.'),
  ('deepseek', 'deepseek-v4-pro', 540000, 2200000,
   'PROVISIONAL. Twice the flash entry. Fallback only — reached when flash is exhausted.');
