...
--
--   revenuecat : provider_subscription_id = provider_payment_id = 스토어 주문번호  → 맞는다
--   lemonsqueezy: 구독 id ≠ 주문 id                                              → 0건
--
-- 레몬스퀴지 구독 환불은 그래서 여전히 영수증에 반영되지 않습니다. 고칠 수 있는 척하지
-- 않고 남겨 둡니다 — 그쪽은 웹훅이 주문 id 를 따로 들고 오므로 매칭 키부터 다른 문제입니다.
-- 오늘 돈이 오가는 경로는 스토어이고, 실제로 깨진 것이 확인된 것도 스토어입니다.
--
