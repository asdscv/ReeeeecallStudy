-- ============================================================================
-- Migration 095 — import_official_deck now consumes p_deck.native_languages.
--
-- Until now the import RPC wrote learning_language but ignored native_languages;
-- the column was populated only by backfill migrations (091/092/094). The
-- official-decks gateway already sends `native_languages` in p_deck (derived by
-- DeckMetadataBuilder = the non-English / mother-tongue side), so newly imported
-- or re-imported official decks were left with stale/NULL native tagging until a
-- backfill re-ran. This migration closes that gap: the RPC writes both the
-- `native_languages TEXT[]` array and the scalar `native_language` (= [1]) on
-- decks and marketplace_listings.
--
-- Faithful reproduction of the 090 body (which itself reproduced 083) + native
-- handling. CREATE OR REPLACE keeps the signature; no return-type change.
--
-- Safety: when p_deck omits/empties native_languages, COALESCE preserves any
-- existing value instead of wiping it. Empty array → NULL via NULLIF.
-- Deploy: DB migration (not OTA) — apply via management SQL / `supabase db push`.
-- ============================================================================
...
BEGIN
  ...
  v_native_langs  := ARRAY(
    SELECT jsonb_array_elements_text(COALESCE(p_deck->'native_languages', '[]'::jsonb))
  );
  ...
  INSERT INTO decks (
    ...
    learning_language, native_languages, native_language
  ) VALUES (
    v_deck_id, v_system_user_id, v_name, v_description, v_template_id,
    v_color, v_icon, 'subscribe', true, 0, v_card_count,
    v_learning_lang, NULLIF(v_native_langs, ARRAY[]::TEXT[]), v_native_langs[1]
  )
  ON CONFLICT (id) DO UPDATE SET
    ...
    native_languages    = COALESCE(EXCLUDED.native_languages, decks.native_languages),
    native_language     = COALESCE(EXCLUDED.native_language, decks.native_language),
    updated_at          = NOW();
  ...
  INSERT INTO marketplace_listings (
    ...
    NULLIF(v_native_langs, ARRAY[]::TEXT[]), v_native_langs[1]
  )
  ON CONFLICT (deck_id) DO UPDATE SET
    ...
    is_active         = true,
    learning_language = EXCLUDED.learning_language,
    native_languages  = COALESCE(EXCLUDED.native_languages, marketplace_listings.native_languages),
    native_language   = COALESCE(EXCLUDED.native_language, marketplace_listings.native_language),
    updated_at        = NOW()
