-- ============================================================================
-- 196: the three things 194 needed and did not have
--
-- Found by auditing the shipped feature rather than by a failure. Each one is
-- invisible until it has already cost something, which is why they are worth
-- closing before the feature has users rather than after.
--
-- ── 1. Abandoned holds had no sweeper that would ever run ───────────────────
--
-- 194 shipped `sweep_ai_quiz_holds` and nothing calls it. pg_cron is NOT installed
-- on this project (migration 127 says so explicitly, and the scheduled work that
-- exists — toss-renew.yml — is a GitHub Actions cron hitting an edge function).
--
-- So a request that dies without reaching its catch block — an edge timeout, a
-- worker eviction — leaves `quiz_units_held` outstanding forever. Reserve compares
-- the balance against the SUM of outstanding holds, so that learner's wallet is
-- permanently short by the abandoned amount, with nothing on any screen to explain
-- it and no way for them to clear it.
--
-- The fix is the pattern this repo already uses where pg_cron is unavailable
-- (mig 106): sweep opportunistically, inside the RPC. And it can be narrower than
-- the global sweeper, because the only person an abandoned hold harms is the person
-- whose balance it blocks — who is, necessarily, the caller. Each reserve settles
-- the CALLER's own stale holds first. Self-healing, no infrastructure, and it costs
-- one indexed lookup on a path that is already doing several.
--
-- ── 2. A learner could not see the allowance they were given ────────────────
--
-- `get_ai_wallet_summary` reports card credits and the daily free CARD count. The
-- 60-unit quiz trial and the daily quiz allowance appear nowhere except inside a
-- quote on the setup screen — so a learner who has not opened quiz cannot discover
-- they have it, and one who has spent it cannot see why the price changed.
--
-- ── 3. Quiz margin was arithmetically hiding card margin ────────────────────
--
-- `get_ai_margin_daily` groups by (day, provider, model). Quiz and card generation
-- both resolve `text` — the same provider and the same model — so they land in ONE
-- bucket. Quiz sells at a list price roughly 20x its cost; card generation runs at
-- the 80% target. Blended, a card-side cost regression is absorbed by quiz's margin
-- and `under_target` never fires. The two are different businesses and have to be
-- read apart.
-- ============================================================================

BEGIN;

-- ── 1) Reserve sweeps the caller's own abandoned holds ──────────────────────
--
-- Body reproduced from 194 with one block added before the balance gate, so this
-- file is the whole current definition rather than a diff to chase.
...
-- ── 2) The wallet says what the learner has for quiz ────────────────────────
...
-- ── 3) Margin is reported per job kind ──────────────────────────────────────
--
-- The added column is the LAST one, and grouping gains one key. Existing callers
-- select by name and keep working; a caller that wants the old blended view groups
-- the result again. The alternative — leaving it blended — means `under_target`
-- can never fire for cards once quiz has any volume, because quiz sells at roughly
-- 20x cost and card generation at the 80% target, in the same provider/model bucket.
...
CREATE OR REPLACE FUNCTION public.get_ai_margin_daily(
  ...
  day date, provider text, model text, job_kind text,
...
COMMENT ON FUNCTION public.reserve_ai_quiz(text, integer, uuid, bigint, uuid, uuid[], uuid, uuid) IS
  'Reserve quiz units as a HOLD (no ledger write). Also settles the callers own abandoned holds first: pg_cron is not installed, sweep_ai_quiz_holds would never run, and a stale hold silently shortens exactly one wallet — the callers.';
