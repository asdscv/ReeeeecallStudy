...
describe('generateSitemapXml', () => {
  const staticPages = [
    { loc: 'https://reeeeecallstudy.com/landing', changefreq: 'weekly' as const, priority: 1.0 },
    { loc: 'https://reeeeecallstudy.com/content', changefreq: 'daily' as const, priority: 0.9 },
  ]

  const contentPages = [
    {
      loc: 'https://reeeeecallstudy.com/content/spaced-repetition-guide',
      lastmod: '2025-06-01T00:00:00Z',
      changefreq: 'monthly' as const,
      priority: 0.8,
    },
    {
      loc: 'https://reeeeecallstudy.com/content/active-recall-techniques',
  ...
  it('should include all static pages', () => {
    const xml = generateSitemapXml(staticPages, [])
    expect(xml).toContain('<loc>https://reeeeecallstudy.com/landing</loc>')
    expect(xml).toContain('<loc>https://reeeeecallstudy.com/content</loc>')
  })

  it('should include all content pages', () => {
    const xml = generateSitemapXml([], contentPages)
    expect(xml).toContain('<loc>https://reeeeecallstudy.com/content/spaced-repetition-guide</loc>')
    expect(xml).toContain('<loc>https://reeeeecallstudy.com/content/active-recall-techniques</loc>')
  })
